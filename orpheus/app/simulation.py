...
ROLE_DIRECTIVES = {
    # Stage 45 — a role is a job, not a volume knob. alpha/beta/gamma described how
    # expensive a generation was, so the "team" was one bot speaking and two repeating
    # it more cheaply. These say what the member is FOR in the discussion.
    "scout": "Ты не спорь. Твоё дело — разобраться: назови, что здесь реально утверждают, и на чём это держится. Короткий уточняющий вопрос или констатация факта, без позиции.",
    "opener": "Ты заходишь первым по существу. Возьми ОДИН конкретный довод из своих и скажи его как собственное живое мнение, привязавшись к детали этого поста. Не ссылайся ни на какие «предыдущие комментарии».",
    "support": "Кто-то уже возразил нашей стороне. Ответь именно на это возражение — не повторяй то, что наши уже сказали, возьми другой довод и разбери конкретное утверждение оппонента.",
    "closer": "Разговор накалён. Сбавь градус: согласись с тем, в чём оппонент прав, и мягко верни к нашей мысли. Без наездов, коротко.",
    # Legacy castes still arrive from older rosters; keep them working.
    "alpha": "Ты заходишь первым по существу — как собственное живое мнение, без ссылок на «предыдущие комментарии».",
    "beta": "Ты усиливаешь мысль союзника: согласись и добавь СВЕЖИЙ довод, не копируя его слова.",
